...
%
% Note: Edited on 01/09/09 by Jesse Amundsen
%       - Disabled hit detection for everything (orthogonal distances,
%         planes and lines)
...
switch lower(geometry)
     ...
     case {'on'}
        ...
        mesh(xgrid,ygrid,zgrid,'EdgeColor','black','FaceAlpha',0,'HitTest','off');
        hold on
        ...
        switch lower(sod)
            case {'off'}
                plot3(X(above,1),X(above,2),X(above,3),'cyan*','HitTest','off');
            case {'on'}    
                plot3(X1',X2',X3','cyan-', X(above,1),X(above,2),X(above,3),'cyano','HitTest','off');
        end   
        nbelow = sum(below);
        ...
        switch lower(sod)
            case {'off'}
                plot3(X(below,1),X(below,2),X(below,3),'blue*','HitTest','off');
            case {'on'}    
                plot3(X1',X2',X3','blue-', X(below,1),X(below,2),X(below,3),'blueo','HitTest','off');
        end        
     case{'off'}
...
%
     ...
     switch lower(visualization)
     case {'on'}
        plot3(endpts(:,1),endpts(:,2),endpts(:,3),'black-','HitTest','off');
        ...
        switch lower(sod)
            case {'off'}
                plot3(X(:,1),X(:,2),X(:,3),'blue*','HitTest','off');
            case {'on'}    
                plot3(X1',X2',X3','cyan-', X(:,1),X(:,2),X(:,3),'blueo','HitTest','off');
        end
        grid;
     case{'off'}
        disp('No visualization.')
